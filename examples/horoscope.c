...
char *astro_write_hms(char *str,double c,const char *deg) {
	...
	if(s>=60) s -= 60,m++;
	if(m>=60) m -= 60,h++;
	sprintf(p,"%d%s%02d\'%02d\"",h,deg,m,s);
	return str;
}
...
int main(int argc,char *argv[]) {
	...
	if(argc>5) sscanf(argv[5],"%lf",&tz);
	if(argc>6) dst = !strcmp(argv[6],"1") || !strcmp(argv[6],"yes")? 1 : 0;

	h = horoscope_new("",lon,lat,tz);
	horoscope_set_time(h,year,month,day,hour,minute,second,dst,1);
...
/*	printf("Longitude: %lf\n"
	       ...
	       "Time: %d-%d-%d %d:%d:%d\n"
	       "Timezone: %lf\n"
	       "DST: %lf\n",lon,lat,year,month,day,hour,minute,second,tz,dst);*/
	...
	printf("\nAstronomy: %d-%02d-%02d %d:%02d, jd(%f), lon(%3.2f), lat(%3.2f)\n",
	...
	for(i=0; i<a->nplanets; i++) {
		p1 = &a->planets[i];
		if(p1->id==ASTRO_ASCENDANT || p1->id==ASTRO_MC) continue;
		...
		printf("%6.2f   %10s  %2d     %6.2f   %s\n",p1->radius,astro_write_hms(deg,p1->speed,"."),p1->constel,a->mphase,zodiac_names[sign]);
	}
...
/*	printf("\nSid. Time: %lf\n"
	       ...
	       "Right Ascension: %lf\n"
	       "Ob: %lf\n"
	       "Latitude: %lf\n",h->sdrlt,h->gmt.jd,h->deltat,h->ra,h->ob,h->la);*/
